...
def GetCallOptions(stk): # {s['Ticker'], s['Price'], s['Change']}
	for retry in [0,1,2]:
		try:
			sym = stk['Ticker']
			stkprice = 0
			...
			#stkprice = 0 # will set price at first option
			stkprice = yfprice
			APIKEY='3p4Ch1P9b-------------idub1N' # Polygon.io  $99/month realtime
			#get option list
			url = 'https://api.polygon.io/v3/reference/options/contracts?underlying_ticker='+sym+'&contract_type=call&expiration_date='+expdate+'&limit=150&sort=strike_price&order=asc&apiKey='+APIKEY
			rsp = requests.get(url, verify=False).json() # all call options for this symbol
			#print(url,'\n\n',rsp)
			###print(rsp) # option list but no bid\ask prices
			data = []
			#get option details
			for cll in rsp['results']:
				if cll['strike_price'] < stkprice: continue # skip ITM
				tkr = cll['ticker']  # O:QQQ220218C00195000
				url = 'https://api.polygon.io/v3/snapshot/options/'+sym+'/'+tkr+'?apiKey='+APIKEY
				rc = requests.get(url, verify=False).json() # call details with price, realtime
...
def GetTickerOptions(stks): # stks is [{s['Ticker'], s['Price'], s['Change']},,,,]
	...
	results = pool.map(GetCallOptions, stks)
	return results

def UpdateStockQuotes(stkdata): # get real-time quotes
	print('UpdateStockQuotes',len(stkdata))
	mbkey = 'yOA0bpDZlcMjK----------------LRayeWK4UGGfTKJn' # https://mboum.com/api/welcome, free account
	quotes = {} # dictionary  symbol:price
	#https://mboum.com/api/v1/qu/quote/?apikey=yOA0DZMjKng------TVjKJn&symbol=AAPL,F
	...
	for sd in stkdata:
		...
		if ctr%50 == 0 or ctr==len(stkdata):  # max 50 symbols per request, 5 requests per second
			url=url[:-1] # trim last comma, else error
			#print(url)
			rsp = requests.get(url, verify=False).json()  #send request
			print('rsp len',len(rsp['data']))
			for rec in rsp['data']:
				earndate = ''
	...
	rsp = url = None
	for sd in stkdata:
		sd['price'] = quotes[sd['sym']]['price'] # overwrite
...
if __name__ == '__main__':
	...
	expdate = GetFridayDate()  #GetThursdayDate()

	printx('Exp Date:', str(expdate))
	...
	IgnoreStk  = ['TNA','TQQQ','SQQQ','UPRO','BRZU','TECL','UDOW','NAIL','HEI-A','MOG-A','XLNX', 'FAS','UCO','GUSH','LABU','LABD','BOIL'] # skip leveraged indexes, very volatile, high risk
	...
	d1 = datetime.now()
	stkdata = GetTickerOptions(stks) # all option data for all stocks, delayed stock quotes
	d2 = datetime.now()
	print('\nOption Time:', d2-d1)
	
	# update stock data with real-time quotes (free, MBoum)
	...
	for trycnt in [1]: # if options error, retry failed stocks
		...
		for stk in stksrt:  # each stock from polygon
			try:
				...
				if not stkcalls: continue # error
