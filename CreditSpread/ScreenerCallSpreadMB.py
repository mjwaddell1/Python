...
from finviz.screener import Screener  # https://github.com/mariostoev/finviz   pip install -U git+https://github.com/mariostoev/finviz
# Can run local copy
#   import sys
#   sys.path.insert(0,'D:/MikeStuff/Python/finviz-master')
#   change imports: finviz.xxx -> xxx
#   from screener import Screener  # local copy with fix, check if github updated

# If install error: invalid TOML
#    download zip from github
#    remove last characters from TOML (git)
#    install from local folder: pip install -e D:\MikeStuff\Python\finviz-master
...
key='58f18d4b-xxxx-xxxx-xxxx-04465a30199b'  # https://github.com/yongghongg/stock-symbol
...
with open('D:/MikeStuff/StockList.json', 'w') as f:
...
def GetStockHistory(sym):  # get history from mboum
    ...
    mbkey = 'yOA0bpDZlcMKngTVrvJG0zUxxxxxxxxxxfLLRayeWK4UGfTVjKJn' # https://mboum.com/api/welcome, free account
    #url='https://mboum.com/api/v1/hi/history/?symbol=F&interval=1d&diffandsplits=true&apikey=yOA0bpDZlcMKngTVrvJG0zUxxxxxxxxxxfLLRayeWK4UGfTVjKJn'
    url='https://mboum.com/api/v1/hi/history/?symbol='+sym+'&interval='+iv+'&diffandsplits=true&apikey='+mbkey
    hdrs={"User-Agent":"Mozilla/5.0,(Windows NT 10.0; Win64; x64),AppleWebKit/537.36,(KHTML, like Gecko),Chrome/110.0.0.0,Safari/537.36"}
...
def GetEarningsDay(sym, maxdays):  # future earnings dates (days from today)
  try:
    url = 'https://ab.tradeui.com/api/earnings.php?symbol='+sym
...
#  quit()
  ...
  #stock_list = Screener(filters=filters, table='screener-views-table', order='ticker')  # Get the performance table and sort it by symbol ascending
  ...
  IgnoreStk  = ['TNA','TQQQ','SQQQ','UPRO','BRZU','TECL','UDOW','NAIL','HEI-A','MOG-A','XLNX', 'FAS','UCO','GUSH','LABU','LABD','BOIL','SOXS','KOLD','DPST'] # skip leveraged indexes, very volatile, high risk
    
  #finviz may be fucked
  if 'Change' in stock_list.data[0].keys():
     stks = [{'Ticker':s['Ticker'], 'Price':s['Price'], 'Change':s['Change']} for s in stock_list.data]  # symbol, price, change only
  else: #by key position
     keylst=list(stock_list.data[0].keys())
     stks = [{'Ticker':s[keylst[1]], 'Price':s[keylst[14]], 'Change':s[keylst[15]]} for s in stock_list.data]  # symbol, price, change only
  ...
  print('    Option bid (sell) price, Hit chance % (past 1 year)')
  print('    Option ask (buy ) price, % to strike price, % profit, % gap (full loss)\n')
  print('  To invest:')
