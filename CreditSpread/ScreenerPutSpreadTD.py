...
TD_KEY = 'FY5PCMC0AN---------ULYURLY' # get your own (free) key from https://developer.tdameritrade.com
...
def GetOptions(sym): # get option chain for symbol
	for x in [1,2,3]:  # loop, 3 tries if connection error
		try:
			...
			url = 'https://api.tdameritrade.com/v1/marketdata/chains?apikey='+TD_KEY+'&symbol='+sym+'&contractType=PUT&strikeCount=50&toDate=' + maxdt
			...
			for dt in rsp['putExpDateMap'].keys():  # ['2021-10-22:2', '2021-10-29:9', '2021-11-05:16', ...
				expdays = dt.split(':') # ExpDate:DaysToExp
				for strk in rsp['putExpDateMap'][dt].keys(): # ['37.0', '38.0', '39.0', '40.0']
					optlst.append({'exp':expdays[0], 
												 'strike':float(strk), 
												 'bid':float(rsp['putExpDateMap'][dt][strk][0]['bid']), 
												 'ask':float(rsp['putExpDateMap'][dt][strk][0]['ask']), 
												 'days':int(expdays[1])})
...
for trycnt in [1,2]: # if options error, retry failed stocks
	...
	for stk in stksrt:  # each stock from finviz
		...
		try:
			...
			stkputs = GetOptions(stk['Ticker'])  # list of dictionaries
			#print(stkputs)
			if not stkputs: # no options returned for this symbol
				stkretry.append(stk) # retry
				continue # no options returned for this symbol
			#print('found puts', stk['Ticker'], pr)
			for put in stkputs: # for each call option in option list
				if skip: break # failed earnings check, move to next stock
				if put['days'] > MaxExpDays: continue # skip these options (too long until expiration) Weekly only
				strike = put['strike']  # option strike price
				#print(put)
				# check if possible short option
				if strike <= pr * 0.2: continue # bad data
				if strike <= pr * (1 - MinBuffer) and put['bid'] >= MinPremium: # 5% under price, premium > $1
					#print(put)
					# scan for possible long option
					for put2 in stkputs: # scan all calls, ignore call closer to ATM
						if put2['days'] != put['days']: continue # same exp date
						...
						#if put2['strike'] <= put['strike'] * 1.10: continue  # must be at least 5% buffer
						if put['bid'] - put2['ask'] <= MinSpread: continue  # premium spread must be at least $1
						if put['bid'] == 0: continue  # useless
						#print(put, '\n', put2)
