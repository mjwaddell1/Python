...
def GetStockNames():
    ...
    sskey = '58f814d4b-753e-xxxx-b72a-04465a03199b'  # https://github.com/yongghongg/stock-symbol
...
mbkey = 'yOA0bZlcMjKngT-------------iTPfLLRayeWKGfTVjKJn'  # https://mboum.com/api/welcome, free account
...
def GetPutOptionsMB(stk):  # MBoum {s['Ticker'], s['Price'], s['Change']}
    retry = 0
    url = ''
    while retry < 3:  # if request fails
        ...
        try:
            ...
            data = []
            # get option details
            for cll in rsp['data']['optionChain']['result'][0]['options'][0]['puts']:
            ...
            print('.', end='')
            return {'sym': sym, 'price': stkprice, 'yprice': stk['Price'], 'ychange': stk['Change'],
                    'yfprice': yfprice, 'puts': data, 'error': ''}  # include yahoo price\change
        except Exception as ex:
...
def GetTickerOptionsMB(stks):  # MBoum - stks is [{s['Ticker'], s['Price'], s['Change']},,,,]
    ...
    results = pool.map(GetPutOptionsMB, stks)
    return results
...
if __name__ == '__main__':
    ...
    optlogfile = "C:/Mike/OptionPutHistorySpread.txt"  # store option results (append)
    ...
    # print(GetHitRate('W', -0.09))
    # print(expdate, daysToExp, expts)
    # quit()
    ...
    IgnoreStk = ['TNA', 'TQQQ', 'SQQQ', 'UPRO', 'BRZU', 'TECL', 'UDOW', 'NAIL',
                 'HEI-A', 'MOG-A', 'XLNX', 'FAS', 'UCO', 'GUSH', 'LABU', 'LABD',
                 'BOIL', 'SOXS', 'KOLD', 'DPST']  # skip leveraged indexes, very volatile, high risk
    ...
    print('  To invest:')
    print('     Buy Put Spread')
    print('        Sell Bid option')
    print('        Buy Ask option')
    print('     Use limit order (Net Credit)')
    print('     MaxLoss = [BuyStrike - SellStrike] * 100 * Quantity')
    print('  Before submit, confirm % return (Proceeds/[BuyStrike - SellStrike])')
    ...
    # For each stock from FinViz, get put option chain
    # Goal is to find 2 put options where short strike is >= 5% under stock price 
    ...
    for trycnt in [1]: # if options error, retry failed stocks
        ...
        for stk in stksrt:  # each stock from MB
            try:
                ...
                if not stkputs: continue # error, no options for this stock
                for pt in stkputs: # for each put option in option list
                    if skip: break # failed earnings check, move to next stock
                    strike = pt['strike']  # option strike price
                    # check if possible short option
                    if strike <= pr * (1 - MinBuffer) and pt['bid'] > MinPremium: # 5% below price (safety margin), premium > 50 cents
                        # scan for possible long option
                        for pt2 in stkputs: # scan all puts, ignore puts closer to ATM
                            if pt2['strike'] >= pt['strike']: continue  # closer to ATM
                            if (pt['strike'] - pt2['strike'])/pt['strike'] < MinStrGap: continue # strikes too close together (if stocks goes ITM, will quickly cross entire spread)
                            if pt['bid'] - pt2['ask'] <= MinSpread: continue  # premium spread must be at least 50 cents
                            if pt['bid'] == 0: continue  # useless put
                            if (pt['bid'] - pt2['ask'])/(pt['strike'] - pt2['strike']) >= MinReturn:  # at least 10% return
                                ...
                                # print option data
                                printx('>>> ',
                                       str(pt['strike']).rjust(8),
                                       str("%.1f" % pt['bid']).rjust(8),
                                       (str(int(GetHitRate(stk['sym'],strike/pr-1)*100))+'%').rjust(8))

                                printx('<<< ',
                                       str(pt2['strike']).rjust(8),
                                       str("%.1f" % pt2['ask']).rjust(8),
                                       pct(strike/pr - 1).rjust(8),
                                       pct((pt['bid'] - pt2['ask'])/(pt['strike'] - pt2['strike'])).rjust(8),
                                       ('    ['+pct((pt['strike'] - pt2['strike'])/pt['strike']).rjust(5)+']'))
