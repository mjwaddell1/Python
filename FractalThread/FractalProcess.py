...
def MandelbrotProcess(xyi):  # generate single column
    x, y, i = xyi
    result = []
...
def MandelbrotSet(x, y):  # use multi-process
    print('Multi Process')
    ...
    for i in range(len(x)):  # all columns
        ...
        if ctr == proc_cnt:  # process 12 columns at a time
            ctr = 0
            results = pool.map(MandelbrotProcess, args_list)  # wait for child processes
            args_list = []
